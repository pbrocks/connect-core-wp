 ...
 * Example usage -
 *   Info notification:
 *      new Admin_Notification( 'Testing Info Notification' ); // OR
 *      new Admin_Notification( 'Testing Info Notification', 'success' );
 *   Warning notification:
 *      new Admin_Notification( 'Testing Warning Notification', 'warning' );
 *   Error notification:
 *      new Admin_Notification( 'Testing Error Notification', 'error' );
 *   Info notification with stack trace:
 *      new Admin_Notification( 'Testing Info Notification', 'success', true );
 **/
class Admin_Notification {

	protected $title = 'Core Admin Notification';
	...
	private function build( $message, $type, $include_backtrace ) {
		...
		$notification = new Admin_Notification_Builder(
			$message,
			$type,
			$backtrace_object,
			$this->title,
...
class Admin_Notification_Builder {
	private $message;
	...
	public function get_notification_html() {
		...
		<div id='message' class='notice <?php esc_html_e( $this->notification_type ); ?>'>
			<p>
				...
				<?php
					echo wp_kses( $this->message, array(
						'a' => array(
							'href' => array(),
							'target' => array(),
						),
		...
		<div class='stack-trace-container'>
			...
			for ( $i = 0; $i < $stack_length; $i++ ) {
				...
				<div class='stack-trace'>
					&#8618; <span class='code'><span class='code-highlight'><?php echo esc_html( $class ); ?></span><?php echo esc_html( $type ); ?><span class='code-highlight'><?php echo esc_html( $function ); ?></span>(<?php

					$args_length = count( $args );
					for ( $j = 0; $j < $args_length; $j ++ ) {

						?><span class='code-highlight'><?php echo esc_html( $args[ $j ] ); ?></span><?php
if ( $j !== $args_length - 1 ) :
	?>, <?php // Output a comma after each non-last argument
						endif;
					}
